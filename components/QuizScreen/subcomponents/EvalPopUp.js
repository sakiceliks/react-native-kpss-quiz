...
const EvalPopUp = ({ explanation, referTo, totalCount, correctAnswers}) => {
  ...
  const motivation = ["Aynen böyle devam!", "Mükemmel gidiyorsun!", "İyi İş!", "Yanıyorsun", "Aferin sana!", "İşte Böyle!", "Bravo!"];
          ...
          <View style={[styles.modalView, {backgroundColor: colors.light, borderColor: 'white'}]}>
            ...
            (
              <>
                ...
                <Text style={[styles.modalText, {color: colors.dark, fontFamily: 'Poppins-Bold', fontSize: 19 }]}>{(!isTraversing)?"Bu Doğru! ":"Bu Doğruydu!"}</Text>
                {(isTraversing)?
                <>
                {(explanation != '')?
                 <Text style={[styles.modalText, {color: colors.dark,}]}>
                 <Text style={{ fontFamily: 'Poppins-Bold' }}>Açıklama:{"\n"} </Text>
            ...
            <>
              ...
              <Text style={[styles.modalText, {color: colors.dark}, { fontFamily: 'Poppins-Bold', fontSize: 19 }]}>{(!isTraversing)?"Yanlış Cevap!":"Bu doğru değildi!"}</Text>
              <Text style={{ color: colors.dark, fontFamily: 'Poppins-Regular', fontSize: 17 }}><Text style={{ fontFamily: 'Poppins-Bold', fontSize: 17 }}>{letters[correctAnswers[shownQuestion] - 1]}</Text> Doğru Cevap</Text>
            ...
            >
              <Text style={[styles.submitText, { color: colors.light }]}>
                {(!isTraversing && (currentQuestion != totalCount-1)) ? ("Sıradaki Soru") : ("Tamam")}
                </Text>
            </Pressable>
          </View>
        </View>
