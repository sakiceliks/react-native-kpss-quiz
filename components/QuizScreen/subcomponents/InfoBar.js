...
export default function InfoBar({ totalCount, correctAnswers}){
  ...
  if (selectedChoices.length > 0) {
   ...
   return (
     ...
     <View>
       <Text style={[styles.infoItem,{backgroundColor: colors.light, color: colors.dark, borderColor: colors.border, marginLeft: 'auto'}]}> <Text style={styles.Q}>Soru</Text> {shownQuestion+1}/{totalCount}</Text>
     </View>
